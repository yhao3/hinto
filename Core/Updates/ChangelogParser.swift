...
final class ChangelogParser {
    ...
    private func extractEntry(from content: String, version: String) -> ChangelogEntry? {
        // Pattern: ## [0.1.0] - 2026-01-01 or ## [0.1.0]
        let pattern = #"## \[\#(version)\](?:\s*-\s*(\d{4}-\d{2}-\d{2}))?\s*\n([\s\S]*?)(?=\n## \[|\z)"#
            .replacingOccurrences(of: "#(version)", with: NSRegularExpression.escapedPattern(for: version))

        log("ChangelogParser: Looking for version \(version), pattern: \(pattern)")
        log("ChangelogParser: Content length: \(content.count)")

        guard let regex = try? NSRegularExpression(pattern: pattern, options: []) else {
            log("ChangelogParser: Failed to create regex")
            return nil
        }

        guard let match = regex.firstMatch(in: content, options: [], range: NSRange(content.startIndex..., in: content))
        else {
            log("ChangelogParser: No match found for version \(version)")
            return nil
        }
